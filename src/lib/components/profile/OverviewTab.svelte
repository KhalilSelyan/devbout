			...
			<div class="space-y-4">
				...
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-2">
						...
						<a
							href={`https://github.com/${userProfileData?.githubUsername}`}
							target="_blank"
							referrerpolicy="no-referrer"
							class="hover:text-gray-500"
						>
							{userProfileData?.githubUsername}
						</a>
					</div>
				</div>
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-2">
						...
						<a
							href={`https://discord.com/users/${userProfileData?.discord}`}
							target="_blank"
							referrerpolicy="no-referrer"
							class="hover:text-indigo-500">{userProfileData?.discord}</a
						>
					</div>
				</div>
				...
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-2">
						...
						<span> {userProfileData?.xpPoints} Total XP</span>
					</div>
				</div>
			</div>
		</CardContent>
		...
		<CardContent>
			<div class="flex flex-wrap items-center gap-2">
				{#if userProfileData?.skills}
					{#each Object.entries(userProfileData.skills) as [skill, level]}
						<Badge class="flex w-fit items-center gap-2 p-2">
							<span>{skill}</span>
							<span>•</span>
							<span>{level}</span>
						</Badge>
